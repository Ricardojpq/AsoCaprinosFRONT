...
export class Animals implements OnInit {
  ...
  ngOnInit() {
    ...
    // Removido: this.loadAnimals(); - PrimeNG Table con lazy loading se encarga automáticamente
  }
  ...
  deleteSelectedAnimals() {
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        Promise.all(deletes.map((obs) => firstValueFrom(obs))).then(() => {
          this.loadAnimals();
  ...
  saveAnimal() {
    ...
    if (
      ...
      this.messageService.add({
        severity: 'error',
        summary: 'Error',
        detail: 'Completa todos los campos obligatorios (Código de Finca, Código de Animal, Nombre, Sexo)',
        life: 3000,
      });
      return;
    }
